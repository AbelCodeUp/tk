    ...
    setPageStyleByDomain(){
        ...
        switch (domain){
            case  'icoachu': //英练邦
                ...
                break;
            case  'gogotalk': //gogotalk
                TkGlobal.format = "gogotalk" ;
                break;
            default:
        ...
        //gogotalk 增加一对一 样式判断
        if(TkGlobal.format == "gogotalk" && TkUtils.getUrlParams('roomtype', window.location.href ) == TkConstant.ROOMTYPE.oneToOne ){
            //gogotalk 增加一对一 样式判断
            $(document.body).removeClass("normal gogotalk").addClass(TkGlobal.format).attr("data-company" , TkGlobal.format );
          }else{

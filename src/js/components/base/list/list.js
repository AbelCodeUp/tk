    ...
    loadListItem(listItemDataArray){
        ...
        const _loadListItemContext = (textContextArray) => {
            ...
            const _handlerAddTextContext = (value , index) => {
                const {className , textContext , id , onClick ,   ...ohterAttrs} = value ;
                listItemContextArray.push(
                    <span key={index} className={"tk-listitem-context "+(className || '')  }  id={id}  onClick={onClick} {...ohterAttrs} >{textContext}</span>
                );
            };

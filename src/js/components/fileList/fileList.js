...
import ButtonDumb from "../base/button/Button";
...
class FileListDumb extends React.Component{
    ...
    loadUserListProps(titleJson ,fileListItemJson ){
        ...
        for(let i=0;i<fileListItemJson.length;i++)
        {
            ...
            let tmpFileListItemJson = {
                className: 'file-container clear-float  add-position-relative '  + (active==1?'active':''),
                id: 'file_list_' + id,
                textContextArray: [
                    {
    ...
    render(){
        ...
        return (
            <TkAppListDumb isMediaUI = {this.props.isMediaUI} show={show} {...fileListProps} {...otherProps}  />
        )
    };
};

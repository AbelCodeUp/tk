...
const TkConstant = {} ;
/*版本号*/
let VERSIONS  = 'v1.0.1' , VERSIONSTIME = '2017090411' ,  DEV = false ;
try{
...
TkConstant.remoteNewpptUpdateTime = 201709212004 ; //远程动态PPT文件更新时间
TkConstant.DEV = DEV ; //系统是否处于开发者状态
TkConstant.debugFromAddress =  TkUtils.getUrlParams('debug') ; //从地址栏设置系统为debug状态
...
let hostname = TkUtils.getUrlParams("host") ||  window.location.hostname ;
let port = protocol === 'http'?80:443 ;
Object.defineProperties(TkConstant.SERVICEINFO, {
    protocol: {
        value: protocol,
        writable: false,
    },
    ...
    joinUrl:{
        value: TkUtils.encrypt( window.location.href ) ,
        writable: false,
    }
});
console.info('encrypt_tk_info:\n' ,TkConstant.SERVICEINFO.joinUrl);
// console.info('encrypt_tk_info:\n' ,TkUtils.decrypt(TkConstant.SERVICEINFO.joinUrl) );
...
/*改变上传文件的类型*/
TkConstant.changeDocumentFileListAccpetArr = (addDocumentFileType , addMediaFileType)=> {
...
TkConstant.bindRoomInfoToTkConstant = (joinRoomInfo)=>{
    ...
    L.Logger.info('joinRoomInfo:' , joinRoomInfo);
    ...
    if(TkConstant.joinRoomInfo.companyid == 10036){ //笨鸟的需要前端删除H5课件
        TkConstant.changeDocumentFileListAccpetArr('zip');
    }
};

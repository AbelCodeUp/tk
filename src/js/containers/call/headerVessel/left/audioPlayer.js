...
class AudioPlayerSmart extends React.Component {
    ...
    playMediaFile(event){
        ...
        let stream = TK.Stream({video: false, audio: true, url:url, extensionId:room.getMySelf().id + ":media", attributes:{filename:filename,fileid:fileid,type:'media',toID:to}});
    ...
    handlerStreamAttributesUpdate(updateEvent){
        ...
        if(isTeacher){
            ...
            if(updateEvent.stream.getAttributes().pause!=undefined) {
                //
                let audio=that.refs.playAudio;
                let playBtn=that.refs.playAudioPauseBtn;
    ...
    handlerRoomConnected(roomEvent){
        ...
        for(let i=0;i<streams.length;i++){
            ...
            if(stream.getAttributes().type==="media" && filename!=="") {
    ...
    playPause(){//播放暂停视频
        ...
        that.playPauseTimer = setTimeout( ()=>{
            if(audio.paused){
                ...
                let stream = that.state.stream;
                if(stream!==undefined && isTeacher)
                    room.controlMedia(stream.getID(),{"type":"pause","pause":true});       //恢复
            }
        } , 200 );
    ...
    volumeOnAfterChange(value){//滑动过程和点击音量条时，设置音量大小
        ...
        //L.Logger.debug("volumeOnAfterChange  value = ", value);
        ...
        //L.Logger.debug("volumeOnAfterChange audio.volume = ", radio);
    ...
    volumeOnBeforeChange(){
        const that = this ;
        let audio=that.refs.playAudio;
        //L.Logger.debug(" volumeOnBeforeChange  audio.volume = ", audio.volume);
    ...
    render() {
       ...
       // L.Logger.debug("audioPlayer  render volume=",this.state.volume);
        ...
        return (
            <div className="lc-audio-container add-position-absolute-top0-left0 clear-float "  id="lc_audio_container"  style={{display:!that.state.open?'none':''}}>
                <audio ref="playAudio" preload="auto" id="" data-audio-id=""  src=""    >
              ...
              <button ref="playAudioPauseBtn"  style={{display:!this.state.isTeacher?'none':'block'}} className="playing-btn play add-fl" id="play_pause_btn_audio"  onClick={that.playPause.bind(that)}></button>
                <span className="audio-progress-container add-fl add-block">
                    <span className="audio-info-vessel">
                        <span className="audio-name add-nowrap">{that.state.title}</span>
                        <span className="audio-time">
                            <time ref="currAudioTime" className="curr-play-time"></time>/<time ref="totalAudioTime" className="total-play-time"></time>
                        </span>
                    </span>
                    <div className="audio-progress-outside" >
                ...
                <span className="audio-volume-container add-fl clear-float add-position-relative" style={{width:'1.7rem'}} >
                    <button ref="audioVolumeBtn" className={"volume-btn add-fl yes"} onClick={that._handlerVolumeMute.bind(that) }></button>
                ...
                <button className="audio-close add-fl"  onClick={that.closeAudioVideo.bind(that)}  style={{display:closeStyle}} ></button>
            </div>
        )
    };
};

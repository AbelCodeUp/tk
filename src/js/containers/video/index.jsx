...
class Video extends React.Component {
    ...
    volumeOnAfterChange(value){//滑动过程和点击音量条时，设置音量大小
        ...
        //L.Logger.debug("volumeOnAfterChange media.volume = ", media.volume);
        if(value===0) {
            media.muted = true;
            that.setState({
                volume: 0,
    ...
    playMediaFile(event){
        ...
        let stream = TK.Stream({video: true, audio: true, url:url, extensionId:room.getMySelf().id + ":media", attributes:{filename:filename,fileid:fileid,type:'media',toID:to}});
    ...
    handlerStreamSubscribed(streamEvent){
        ...
        if(!streamEvent.stream.video)  //mp3,动态PPT 直接返回
            return;
       // L.Logger.debug('stream-media_Subscribed =', this.getRoomType());
        if (streamEvent) {
    ...
    setProgress(stream,isTeacher){
        let that = this;
        let progressBar = that.refs.progressBar;

        let position = stream.getAttributes().position;
        if(position === undefined)
            position = 0;
    ...
    handlerStreamAttributesUpdate(updateEvent){
        ...
        if(isTeacher) {  //老师端
            ...
            //L.Logger.debug('stream-media_老师端 =', disabled);
            ...
            that.setProgress(updateEvent.stream, true);
    ...
    handlerStreamRemoved(streamEvent){
        ...
        if(!streamEvent.stream.video)  //mp3,动态PPT 直接返回
        {
            //L.Logger.debug('handlerStreamRemoved 1 =', "test");
            return;
        }

        //L.Logger.debug('handlerStreamRemoved 2 =', "test");
		    		...
		    		<div className="video-controller">
						...
						<div className="video-func-btn-container clear-float">
							...
							<span className="button-right add-fr clear-float">
								...
								<button className="full-btn no add-fl"></button>
							</span>

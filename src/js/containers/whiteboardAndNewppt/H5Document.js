...
class H5Document extends React.Component{
    ...
    handlerRoomPubmsg(pubmsgDataEvent){//room-pubmsg事件：动态h5文档处理
        ...
        switch(pubmsgData.name) {
            ...
            case "H5DocumentAction": //h5文件动作
                that.h5Frame.postMessage(JSON.stringify(pubmsgData.data),'*');
                break;
            default:
                break;
        }
    ...
    handlerIframeMessage(event){ //iframe框架消息处理函数
        ...
        if( event.data ){
            ...
            if (data.method) {
                L.Logger.info("[h5]receive remote iframe data form "+ event.origin +":",  event );
                switch (data.method) {
    ...
    render() {
        ...
        return (
            <div id="h5FileWrap" style={{display:fileTypeMark == "h5document" ? "block": "none"}} className="h5-file-wrap add-position-absolute-top0-left0" >
